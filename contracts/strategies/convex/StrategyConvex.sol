...
abstract contract StrategyConvex is Strategy {
    ...
    uint internal immutable INDEX; // index of token
    ...
    */
    ...
    function _totalAssets() internal view override returns (uint total) {
        ...
        total = lpBal.mul(CURVE.get_virtual_price()) / (MUL * 1e18);
    ...
    function _harvest() internal override {
        ...
        for (uint i = 0; i < rewards.length; i++) {
            ...
            if (bal > 0) {
                _swap(reward.dex, reward.addr, address(token), bal);
            }
        }
    }
